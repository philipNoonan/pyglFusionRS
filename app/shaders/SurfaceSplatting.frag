...
void main(void)
{
	...
	vec3 l = normalize(vec3((vec2(gl_FragCoord) - vec2(K[2][0], K[2][1])) / vec2(K[0][0], K[1][1]), 1.0f));
    ...
    gl_FragData[0] = vec4((gl_FragCoord.x - K[2][0]) * z * (1.f / K[0][0]), (gl_FragCoord.y - K[2][1]) * z * (1.f / K[1][1]), z, gsData.x);
