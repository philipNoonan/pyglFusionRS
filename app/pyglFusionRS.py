...
def main():
    ...
    d2c, c2d, K, invK, colK, depthScale, width, height, colWidth, colHeight = camera.start(useLiveCamera)
...
#        'iters' : (2, 5, 10),
    ...
    while not glfw.window_should_close(window):
        ...
        try:
            depthData, colorData = camera.getFrames(useLiveCamera)
            ...
            if colorData is not None:
                ...
                glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, int(cameraConfig['colorWidth']), int(cameraConfig['colorHeight']), GL_RGB, GL_UNSIGNED_BYTE, np.asarray(colorData))

            if depthData is not None:
                glActiveTexture(GL_TEXTURE1)
                glBindTexture(GL_TEXTURE_2D, textureDict['rawDepth'])
                glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, int(cameraConfig['depthWidth']), int(cameraConfig['depthHeight']), GL_RED, GL_UNSIGNED_SHORT, np.asarray(depthData))
        ...
        if imgui.is_mouse_clicked():
            if not imgui.is_any_item_active():
                ...
                clickedDepth = np.asarray(depthData)[int(yPos+0.5), int(xPos+0.5)] * cameraConfig['depthScale']
